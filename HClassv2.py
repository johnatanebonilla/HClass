...
def classify_normativa_sheet(output_xlsx_path):
    ...
    # Create new dataframes for plural and singular classifications
    normativa_plur = pd.DataFrame(columns=df.columns)
    normativa_sing = pd.DataFrame(columns=df.columns)

    # Function to classify the rows
    ...
    for index, row in df.iterrows():
        ...
        if classification == 'plur':
            plur_rows.append(row)
        else:
            sing_rows.append(row)

    normativa_plur = pd.concat([normativa_plur, pd.DataFrame(plur_rows)], ignore_index=True)
    normativa_sing = pd.concat([normativa_sing, pd.DataFrame(sing_rows)], ignore_index=True)

    # Load the original file to retain other sheets
    all_sheets = pd.read_excel(output_xlsx_path, sheet_name=None)  # Load all sheets

    # Add 'id' column to all sheets if it doesn't exist
    for sheet_name, sheet_df in all_sheets.items():
        if 'id' not in sheet_df.columns:
            sheet_df.insert(0, 'id', range(1, len(sheet_df) + 1))
        all_sheets[sheet_name] = sheet_df

    # Save the updated dataframes to a new Excel file
    output_file_path = output_xlsx_path  # Overwrite the original file
    with pd.ExcelWriter(output_file_path, engine='openpyxl') as writer:
        # Write all the original sheets back
        for sheet_name, sheet_df in all_sheets.items():
            sheet_df.to_excel(writer, sheet_name=sheet_name, index=False)
        # Overwrite the 'normativa' sheet
        df.to_excel(writer, sheet_name='normativa', index=False)
        normativa_plur.to_excel(writer, sheet_name='normativa_plur', index=False)
        normativa_sing.to_excel(writer, sheet_name='normativa_sing', index=False)

    print(f"Updated file '{output_file_path}' created successfully with all original sheets and added normativa_plur, and normativa_sing sheets.")
...
def main(parquet_folder, output_folder, spacy_model):
    ...
    # Clasificar la hoja normativa
    classify_normativa_sheet(output_xlsx_path)
